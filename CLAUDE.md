...
- **Ship** — `main` is the release channel for both clients. Load the repo-local `release` skill for releases; `scripts/release.sh` bumps both manifests, commits, tags `v<version>` and pushes. It requires a clean tree on `main`; `sh scripts/release.sh patch -n` previews a patch release without publishing.
