...
- **Share** — `bun run scripts/promote.ts <skill>` copies one skill into the public [vimulatus/skills](https://github.com/vimulatus/skills) checkout at `../skills` (or `SKILLS_REPO`), with `Vasu` rewritten to `the user` and `public/<skill>.patch` applied where the public copy differs, and commits it there. When a patch stops applying, `--stage` gives a dir to resolve and `--patch <dir>` rewrites it. `scripts/release.sh` in that checkout pushes `main`, which is what consumers install from.
