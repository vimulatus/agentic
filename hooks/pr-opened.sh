...
jq -n --arg u "$url" '{
  hookSpecificOutput: {
    ...
    additionalContext: ("You opened \($u). Load the `pr` skill, read its references/watch.md, and arm the watch. A worker told to open it and return skips this: the caller watches.")
  }
}'
