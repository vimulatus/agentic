...
Stay in the session until you report ready, or you are **blocked**.
...
`TaskStop` the watch. Report the PR URL, every thread and comment with what you did and whether it is open, every check that went red and what fixed it, and the issues the PR closes. Then wait. Vasu merges.
...
```bash
${CLAUDE_SKILL_DIR}/scripts/close-parent.sh <N>    # for each ticket the PR closed
```
