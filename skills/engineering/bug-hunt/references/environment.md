...
You make the app reachable for the hunters. Read `<evidence-dir>/SKILL.md`, then run `agent-browser skills get core`; the server, session and auth sections of that skill are the rules here.
...
2. No listener: find the run command in the project's `## Ship` section or its scripts, and start it **detached**, so it outlives you and your shell:

   ```bash
   nohup <run command> > "<task-dir>/server.log" 2>&1 &
   echo $! > "<task-dir>/server.pid"; disown
   ```

   Then, in a **separate** shell call, probe the port again until the app answers. A start that dies with the shell call shows up here, not ten minutes later under a hunter. Return `startedBy: "me"` and the stop command `kill $(cat <task-dir>/server.pid)`.
