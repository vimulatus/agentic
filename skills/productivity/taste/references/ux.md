...
| A new item joins a list | Scroll it into view and centre it, with a brief mark so the eye lands on it. When it sits at the end and the page is too short to centre it, add bottom padding for the scroll, then remove the padding once the reader scrolls or the next change lands. Reduced motion jumps instead of scrolling |
