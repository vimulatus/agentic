...
- An item expanded from a grid returns to its cell. When the page scrolled meanwhile, the exit tracks the cell's current position, and the entrance tracks it the same way.
- A button that morphs into a field keeps the cursor steady until the morph ends: the field takes `pointer-events: none` while it transitions. The button's label carried the meaning, so the field's placeholder can stand as its visible label; the field still gets an accessible name.
