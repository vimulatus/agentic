# Surface: Decisions (ADRs)

An ADR records *that* a decision was made and *why* — so a future reader can reason about how things came to be without asking a human. The value is in the rationale, not in filling out sections.
...
- **Technology choices that carry lock-in.** Database, message bus, auth provider, deployment target — just the ones that would take a quarter to swap out, not every library.
