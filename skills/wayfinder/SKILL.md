...
- **Grilling** (HITL): Conversation via the /grilling and /documentation skills, one question at a time. The default case.
...
1. **Name the destination.** Run a `/grilling` and `/documentation` session to pin down what this map is finding its way to — the spec, decision, or change. The destination fixes the scope, so it's settled first.
...
3. Resolve it — **zoom as needed**: fetch the full body of any related or closed ticket on demand; invoke the skills the `## Notes` block names. If in doubt, use `/grilling` and `/documentation`.
